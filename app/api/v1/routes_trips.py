...
async def create_trip(
    payload: TripRequest, request: Request, service: TripService = Depends(get_service)
) -> CreateTripResponse:
    """接单即返回，规划在后台跑。
    ...
    """
    trip_id = service.create(payload)
    log.info("已受理行程", extra={"trip_id": trip_id, "to": payload.destination_city})
...
@limiter.limit(settings.rate_limit_create)
async def parse_prompt_endpoint(payload: ParseRequest, request: Request) -> TripDraft:
    ...
    """
    draft = await parse_prompt(payload.prompt)
    log.info(
        "解析需求",
